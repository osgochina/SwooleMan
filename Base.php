...
class Base
{
    ...
    protected static $_availableEventLoops = array(
        'swoole' => '\SwooleMan\Events\SwEvent',
        'libevent' => '\SwooleMan\Events\Libevent',
        'event'    => '\SwooleMan\Events\Event'
    );
    ...
    protected static function getEventLoopName()
    {
        ...
        if ($loop_name) {
            if (interface_exists('\React\EventLoop\LoopInterface')) {
                switch ($loop_name) {
                    case 'libevent':
                        self::$eventLoopClass = '\SwooleMan\Events\React\LibEventLoop';
                        break;
                    case 'event':
                        self::$eventLoopClass = '\SwooleMan\Events\React\ExtEventLoop';
                        break;
                    default :
                        self::$eventLoopClass = '\SwooleMan\Events\React\StreamSelectLoop';
                        break;
                }
            } else {
            ...
            self::$eventLoopClass = interface_exists('\React\EventLoop\LoopInterface')? '\SwooleMan\Events\React\StreamSelectLoop':'\SwooleMan\Events\Select';
        }
        return self::$eventLoopClass;
    }
    ...
    protected static function writeStatisticsToStatusFile()
    {
        ...
        if (self::$_masterPid === posix_getpid()) {
            ...
            file_put_contents(self::$_statisticsFile,
                "---------------------------------------GLOBAL STATUS--------------------------------------------\n");
            file_put_contents(self::$_statisticsFile,
                'SwooleMan version:' . Worker::VERSION . "          PHP version:" . PHP_VERSION . "\n", FILE_APPEND);
